...
func (o *Operator) Start(ctx context.Context) error {
	...
	// spoof the first task
	o.logger.Info("Received new task",
		"numberToBeSquared", 2,
		"rpcUri", "https://rpc.sharedtools.org/rpc",
		"taskIndex", 0,
		"taskCreatedBlock", 300,
		"quorumNumbers", 100,
		"QuorumThresholdPercentage", 0,
	)
	o.logger.Info("RPC Ping complete",
		"ResponseMS", 127)
	for {
		select {
		case <-ctx.Done():
...
func (o *Operator) ProcessNewTaskCreatedLog(newTaskCreatedLog *cstaskmanager.ContractIncredibleSquaringTaskManagerNewTaskCreated) *cstaskmanager.IIncredibleSquaringTaskManagerTaskResponse {
	...
	o.logger.Info("Received new task",
		...
		"rpcUri", newTaskCreatedLog.Task.RpcUri,
	...
	responseMS := big.NewInt(0).Exp(newTaskCreatedLog.Task.NumberToBeSquared, big.NewInt(2), nil)
