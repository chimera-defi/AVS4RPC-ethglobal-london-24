...
func (agg *Aggregator) Start(ctx context.Context) error {
	...
	_ = agg.sendNewTask(big.NewInt(taskNum), string("https://rpc.sharedtools.org/rpc"))
	taskNum++
	...
	for {
		...
		case <-ticker.C:
			err := agg.sendNewTask(big.NewInt(taskNum), string("https://rpc.sharedtools.org/rpc"))
			taskNum++
			if err != nil {
...
func (agg *Aggregator) sendNewTask(numToSquare *big.Int, rpcURI string) error {
	agg.logger.Info("Aggregator sending new task", "numberToSquare", numToSquare, "rpcURI", rpcURI)
	// Send number to square to the task manager contract
	newTask, taskIndex, err := agg.avsWriter.SendNewTaskNumberToSquare(context.Background(), numToSquare, "https://rpc.sharedtools.org/rpc", types.QUORUM_THRESHOLD_NUMERATOR, types.QUORUM_NUMBERS)
	if err != nil {
		agg.logger.Error("Aggregator failed to send number to square", "err", err, "rpc", rpcURI)
		return err
	}
