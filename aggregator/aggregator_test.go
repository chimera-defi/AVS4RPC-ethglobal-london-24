...
func TestSendNewTask(t *testing.T) {
	...
	mockAvsWriterer.EXPECT().SendNewTaskNumberToSquare(
		context.Background(), NUMBER_TO_SQUARE_BIG_INT, string("https://rpc.sharedtools.org/rpc"), types.QUORUM_THRESHOLD_NUMERATOR, types.QUORUM_NUMBERS,
	...
	err = aggregator.sendNewTask(NUMBER_TO_SQUARE_BIG_INT, string("https://rpc.sharedtools.org/rpc"))
	assert.Nil(t, err)
}
