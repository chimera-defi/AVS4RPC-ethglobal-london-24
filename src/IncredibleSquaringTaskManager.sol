    ...
    IIncredibleSquaringTaskManager
{
    ...
    function createNewTask(
        ...
        string calldata rpc_uri,
        uint32 quorumThresholdPercentage,
        bytes calldata quorumNumbers
    ) external onlyTaskGenerator {
        Task memory newTask;
        newTask.numberToBeSquared = numberToBeSquared; // hardcode 1 to minimize code changes?
        newTask.rpc_uri = rpc_uri;
    ...
    // custom new task 
    // rpc check for rpc incentives 
    //  validators register their rpc. tasks check if rpcs are valid
    ...
    function raiseAndResolveChallenge(
        ...
        uint256 responseMS = taskResponse.responseMS;
        string calldata rpc_uri = task.rpc_uri;
        // some logical checks
        require(
        ...
        uint256 response_max_delay = 1000; // 1000ms max rpc response delay
        ...
        bool isResponseCorrect = (actualSquaredOutput ==
            ...
            && taskResponse.responseMS <= response_max_delay 
            // TODO: debugme Error (2271): Operator == not compatible with types string calldata and string calldata
            // && task.rpc_uri ==  rpc_uri
            );
